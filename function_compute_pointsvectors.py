...
def compute_pointsvectors_2articulations(a, b, c, width, sigma):
    ...
    nb_bc = int(bc_norm / sigma) +1
    nb_bc_orth = int(width / sigma) +1

    points = []

    for i in range(nb_ab + 1):
        for j in range(nb_ab_orth + 1):
            points_ij = [a[u] + i*sigma*vector_ab_unit[u] + j*sigma*vector_ab_norm_orth[u]
                           for u in range(dim)]
            points.append(points_ij.copy())

    for i in range(1, nb_bc + 1):
        for j in range(1, nb_bc_orth + 1):
            points_ij = [b[u] + i*sigma*vector_bc_unit[u] + j*sigma*vector_bc_norm_orth[u]
                           for u in range(dim)]
            points.append(points_ij.copy())
